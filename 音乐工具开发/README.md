...
完整功能、连续蛋白质表现层、音高策略、调式、算法公式、操作步骤、六声部配器、GVR 规则和科研边界见 `BioSound_GVR生物物理表现层增强版平台说明.docx`。平台生成结果后也可在“试听与导出”页直接下载该手册。
...
flowchart LR
    ...
    F --> E
    E -->|最终通过| G[发布门控]
    G --> S[SoundFont / FluidSynth]
    S --> H[WAV]
    G --> H2[MIDI / MusicXML / PDF]
...
- `biomusic/features.py`：疏水性、电荷、侧链质量、相对 SASA、B-factor、主链二面角、PDB 接触度、声像和粗粒化 NMA。
- `biomusic/codec.py`：DNA/RNA 四进制、蛋白质 21 进制分块与康托排名/逆排名、严格解码。
- `biomusic/mapping.py`：三种音高策略、连续表现曲线、古典配器、空间位置与可选十二音列生成。
- `biomusic/gvr.py`：验证、局部修复、最终发布检查。
- `biomusic/soundfont.py`：验证并加载 SF2，以 FluidSynth 调度独立声部、MIDI CC 和 NMA 采样背景。
- `biomusic/synth.py`：SoundFont 不可用时的程序合成回退与 QC 低通滤波。
...
- 全原子 PDB → 内置 Shrake–Rupley 近似相对 SASA；CA-only 文件明确降级为接触度暴露代理。
- N–CA–C 主链原子 → φ/ψ 圆周角变化和连续刚度；不足时使用 PDB HELIX/SHEET 基线。
- 二级结构与刚度 → 时值、gate ratio 和连奏/断奏。
- 侧链质量 → MIDI CC74 和网页逐音符亮度滤波。
- 相对 SASA × 亲水性 → CC91 混响、CC93 合唱与空间宽度。
- CA B-factor → CC1 调制深度；预测结构中该列可能是置信度，必须核对来源。
- CA 坐标的单位弹簧各向异性网络模型 → 前三个非刚体模态的低频古典声部背景。
...
2. `H_scale`：推荐调式模式下，V1–V6 和 NMA 背景的实际音级都必须属于“主音 + 所选音阶”的集合；越界时投影到最近的调式内音级并复检。
...
1. **生物物理调式映射（推荐）**：以音乐可听性为优先，符号与连续理化特征分别进入调式、节奏、力度、奏法、CC 和空间。用户可选 12 个主音以及大调、自然/和声/旋律小调、七种教会调式、五声、布鲁斯、全音、八音和半音体系。对位按调式音级移动，竖琴按调式内一/三/五级构造，NMA 低音也量化到同一集合。
2. **文献氨基酸映射（复现）**：使用 `pitch_mapping.csv`，用于对照 Spinning Melodies；扩展残基与文献事实分开标记。
3. **可逆十二音列载体（实验）**：用于需要音高载体级严格往返的编码实验，不作为默认创作方式。

推荐模式和文献模式会把规范化原序列、长度、类型与 SHA-256 校验写入 GVR JSON、MusicXML 和 MIDI；因此完整产物可恢复原符号串，但不能声称“裸音高本身可逆”。宿主软件若删除元数据，恢复保证随之失效。
...
平台可严格解码自身导出的 GVR JSON、MusicXML 和 MIDI。若实验模式任一块不是 0–11 的完整排列、排名超出数据域、块数不符或校验和改变，解码器会报错。十二音列只占三种策略之一；WAV 不属于符号恢复载体。

## 六声部古典配器体系

平台不使用颗粒、金属或电子合成器标签。默认六声部为：生物主旋律、小提琴对位、大提琴结构低音、圆号和声场、中提琴内声部、竖琴结构重音。网页 WAV 默认由 FluidSynth 读取本地 SF2 真实采样；MIDI 为格式 1，每声部独立轨道、通道和持久乐器；MusicXML 为每声部独立 Part 和谱表。若需要出版级真实感，请把 MIDI/MusicXML 导入 MuseScore Muse Sounds、专业管弦乐音源或由真实演奏者录制。

### 轻量 SoundFont 采样器

- `TimGM.sf2`（5.69 MiB，GPLv2）：长笛、双簧管、大管、小提琴、中提琴、大提琴、圆号及 NMA 低音背景。
- `Clarinet-20190818.sf2`（7.05 MiB，CC0）：单簧管主旋律。
- `ConcertHarp-small-20200702.sf2`（8.81 MiB，CC0）：竖琴主旋律和结构重音。

每个声部由独立 FluidSynth 通道播放，CC1/10/74/91/93 和 gate ratio 会按事件送入采样器；除 CC10 与 note-off 外，具体 SF2 是否定义并响应相应调制器取决于该音色库。平台会检查 `RIFF/sfbk` 文件头；SF2 缺失或损坏时才记录错误并降级到程序合成。文件来源、许可和 SHA-256 见 `assets/soundfonts/THIRD_PARTY_NOTICES.md`。

## 输出

- WAV：默认 44.1 kHz、16 位立体声 SoundFont 采样；主动选择程序合成回退时为 22.05 kHz。
- MIDI：标准类型 1 文件，包含速度轨、每声部独立乐器轨、CC1/10/74/91/93、奏法门限与嵌入式序列载荷。
...
- GVR JSON：最终检查、修复历史、NMA、音频参数、符号载荷；实验模式另含康托音列载体块。
...
测试覆盖 FASTA 完整导出、全部 16 种调式在 C 与 C♯主音上的严格音级审计、多轨/多 Part 编配、蛋白质连续 CC、JSON/MusicXML/MIDI 符号载荷恢复、可选康托往返与越界拒绝、PDB 空间/NMA 调式量化和 CSV QC 滤波。
